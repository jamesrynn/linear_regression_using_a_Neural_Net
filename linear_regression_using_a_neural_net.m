...
for iterations = 2:Niter
    ...
    % Forward pass
    [a2, D2] = my_activate(x(i), W_stochastic(iterations-1), b_stochastic(iterations-1), act_fn);
...
for iterations = 2:Niter
    ...
    for i = 1:mini_batch
        ...
        % Forward pass
        [a2, D2] = my_activate(x(perm(i)), W_mini(iterations-1), b_mini(iterations-1), act_fn);
...
for iterations = 2:Niter
    ...
    for i = 1:length(x)
        ...
        % Forward pass.
        [a2, D2] = my_activate(x(i), W_batch(iterations-1), b_batch(iterations-1), act_fn);
...
plot(x,y,'xk', 'markersize',10, 'linewidth',2)
hold on
plot(x, my_activate(x, W_stochastic(end), b_stochastic(end), act_fn), 'linewidth',1.5)
plot(x, my_activate(x, W_mini(end),       b_mini(end),       act_fn), 'linewidth',1.5)
plot(x, my_activate(x, W_batch(end),      b_batch(end),      act_fn), 'linewidth',1.5)
hold off
